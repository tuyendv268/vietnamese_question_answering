...
class Loss(Module):
    ...
    def contrastive_loss(self, labels, logits, masks, temperature=1):
        ...
        logits = torch.masked_fill(input=torch.exp(logits), mask=~masks, value=-1000)
        loss = self.cre(logits, labels)
        
        return loss
    
    def caculate_cosin_loss(self, labels, query_embeddings, context_embeddings, masks, temperature=0.1):
        batch_size, n_passage_per_query = labels.shape[0], labels.shape[1]
        context_embeddings = context_embeddings.view(batch_size, n_passage_per_query, -1)
        query_embeddings = query_embeddings.unsqueeze(1)
        
        logits = [pairwise_cosine_similarity(query_embedding, context_embedding) for query_embedding, context_embedding in zip(query_embeddings, context_embeddings)]
