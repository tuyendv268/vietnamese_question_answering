...
if __name__ == "__main__":
    if True:
        ...
        ds_path = 'data/tmp/indexed_ds_example'
        size = 1000
        items = [{"a": np.random.normal(size=[10000, 10]),
                "b": np.random.normal(size=[10000, 10])} for _ in range(size)]
        print(len(items))
        builder = IndexedDatasetBuilder(ds_path)
        args = [{"item": item} for item in items]
        for i, item in multiprocess_run_tqdm(__test, args, num_workers=4, fork_func=None, desc="Build indexed dataset"):
    ...
    if False:
        ds_path = "data/binary/tester/train_1"
        ds = IndexedDataset(ds_path)
        print(len(ds))
        example = ds[0]
