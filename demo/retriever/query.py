...
from demo.reranker.utils import load_index, norm_text, preprocess_text
import torch
...
@torch.no_grad()
def get_topk_passages(query, top_k=2):        
    preprocessed_query = preprocess_text(query)
    segmented_query = " ".join(segmenter.word_segment(preprocessed_query))
    ...
    for index in range(embeddings.shape[0]):
        score = torch.nn.functional.cosine_similarity(query_embedding[0], embeddings[index], dim=-1)
        
        scores.append(score)
    scores = torch.vstack(scores).view(-1)
    
    ranked = torch.topk(scores, k=top_k)
    topk_index = ranked.indices
    topk_scores = ranked.values
    
    topk_text = [passages[rank] for rank in topk_index]
